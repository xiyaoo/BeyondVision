...
def MotorStupe() :
    ...
    while none :
        ...
        if  Motor_ID==3 :
            ...
            Motor_ser.write(command)
            time.sleep(0.1)
            command = [0xFF,0x09,0x01,0x01,0x00]
            Motor_ser.write(command)
            Motor_ser.flush()
            Motor_ser.flushInput()
            time.sleep(10)
...
def SKP20_test() :
    i=0
    j=0
    SKP20_ser = serial.Serial("/dev/ttyAMA0",9600,timeout=1)
    time.sleep(0.1)
    SKP20_ser.close
    SKP20_ser.open
    command = [0xFF,0x0b,0x01,0x01,0x00]
    SKP20_ser.write(command)
    #SKP20_ID=int(input("输入角度步长（2.5度:1;5度：2；10度：3；30度：4：\n"))
    SKP20_Sum=int(input("输入扫描次数（1~100次）：\n"))
    ...
    while i<=SKP20_Sum :
        ...
        SKP20_ser.write(command)
        time.sleep(0.1)
        command = [0xFF,0x09,0x01,0x03,0x00]#45
        SKP20_ser.write(command)
        time.sleep(20)
        print (i)
